...
class TrustNetworkProcessor {
  ...
  computeNetworkReputation(
    ...
    const pathOpinions = validPaths.map((path) => this.computePathOpinion(path));

    // 3. Apply path weighting strategy
    const weightedOpinions = this.applyPathWeighting(pathOpinions, aggregationStrategy);
  ...
  private computePathOpinion(path: TrustPath): SubjectiveOpinion {
    ...
    for (let i = 1; i < path.opinions.length; i++) {
      ...
      const decayedOpinion = this.applyPathDecay(path.opinions[i], i, path.weights[i]);
...
class HierarchicalEBSLProver {
  ...
  async generatePartitionedProof(
    ...
    for (const community of communities) {
      const communityReputation = await this.computeCommunityReputation(userAddress, community);

      const communityProof = await this.generateCommunityProof(communityReputation, community.id);
...
describe("EBSL Algorithm Properties", () => {
  it("should satisfy commutativity", () => {
    property(generateValidOpinion(), generateValidOpinion(), (op1, op2) => {
