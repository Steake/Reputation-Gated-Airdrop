...
test.describe("Anonymous Claim Flow E2E", () => {
  ...
  test("should generate identity, create anonymous proof, and submit successfully", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=Anonymous identity generated and commitment stored")
    ...
    await expect(
      page.locator("select").locator('option:selected:has-text("Anonymous Membership Proof")')
    ).toBeVisible();
    ...
    await expect(page.locator("text=ZK anonymous proof generated successfully!")).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(page.locator("text=Reputation anonymous Verified On-Chain!")).toBeVisible({
      timeout: 10000,
    });
