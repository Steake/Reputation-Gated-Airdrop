...
test.describe('Cross-Chain Wallet and Claim Flows', () => {
  ...
  test('should generate and submit proof on Polygon Mumbai (mock)', async ({ page }) => {
    ...
    await expect(page.locator('text=Reputation exact verified on-chain on Polygon Mumbai!')).toBeVisible({ timeout: 10000 });
  ...
  test('should switch chains and claim with bridged proof simulation', async ({ page }) => {
    ...
    await expect(page.locator('text=Claim successful with bridged proof')).toBeVisible({ timeout: 10000 });
  });
