...
test.describe('Navigation and Routing', () => {
  ...
  test('should handle 404 errors gracefully', async ({ page }) => {
    ...
    const isErrorPage = await page.getByText('Error').isVisible().catch(() => false);
    const isHomePage = await page.getByRole('heading', { name: /Claim Your Reputation/i }).isVisible().catch(() => false);
