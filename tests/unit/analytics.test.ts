...
import { analyticsStore, trackProofGenStart, trackProofGenDuration, trackProofGenSuccess, trackClaimAttempt, trackClaimSuccess, trackAttestationQuery } from '../../src/lib/stores/analytics';
...
describe('Analytics Store', () => {
  ...
  it('should track events and persist to localStorage', () => {
    ...
    expect(mockLocalStorage.setItem).toHaveBeenCalledWith(
      'analytics_events',
      JSON.stringify(expect.arrayContaining([expect.objectContaining({
  ...
  it('should limit persisted events to last 100', () => {
    ...
    expect(mockLocalStorage.setItem).toHaveBeenCalledWith(
      'analytics_events',
      expect.stringContaining(JSON.stringify(expect.arrayContaining([
        expect.objectContaining({ eventType: 'claimAttempt', metadata: { score: 100, community: 'community100' } })
      ])))
    );
    // Should not include the first event
    expect(mockLocalStorage.setItem).not.toHaveBeenCalledWith(
      expect.stringContaining('score: 0')
    );
  });
