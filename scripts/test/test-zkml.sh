...
yarn test:unit 2>&1 | tee test-results/unit-tests.log

# 2. Build Tests
echo "🏗️ Testing build process..."
yarn build 2>&1 | tee test-results/build.log

# 3. Lint Tests
echo "🔧 Running linting checks..."
yarn lint 2>&1 | tee test-results/lint.log || echo "⚠️ Linting issues found, check test-results/lint.log"
...
if command -v npx playwright &> /dev/null; then
    ...
    yarn test:e2e 2>&1 | tee test-results/e2e.log || echo "⚠️ E2E tests failed, check test-results/e2e.log"
else
