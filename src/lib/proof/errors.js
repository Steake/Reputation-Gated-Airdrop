...
export class ProofGenerationError extends Error {
    ...
    metadata;
    originalError;
    constructor(message, type, severity = ProofErrorSeverity.MEDIUM, recoverability = ProofErrorRecoverability.RETRYABLE, metadata = {}, originalError) {
        super(message);
...
export class ResourceOptimizationStrategy {
    canRecover(error) {
        return (error.type === ProofErrorType.OUT_OF_MEMORY ||
...
export class ErrorClassifier {
    ...
    static classify(error, context) {
        ...
        if (errorMessage.includes("circuit") &&
            ...
            return new ProofGenerationError(errorMessage, ProofErrorType.CIRCUIT_COMPILATION_FAILED, ProofErrorSeverity.HIGH, ProofErrorRecoverability.FALLBACK_AVAILABLE, context, originalError);
        }
        if (errorMessage.includes("circuit") && errorMessage.includes("load")) {
            return new ProofGenerationError(errorMessage, ProofErrorType.CIRCUIT_LOAD_FAILED, ProofErrorSeverity.MEDIUM, ProofErrorRecoverability.RETRYABLE, context, originalError);
        }
        // Memory errors
        if (errorMessage.includes("memory") || errorMessage.includes("heap")) {
            return new ProofGenerationError(errorMessage, ProofErrorType.OUT_OF_MEMORY, ProofErrorSeverity.CRITICAL, ProofErrorRecoverability.FALLBACK_AVAILABLE, context, originalError);
        }
        // Timeout errors
        if (errorMessage.includes("timeout") || errorMessage.includes("timed out")) {
            return new ProofGenerationError(errorMessage, ProofErrorType.PROOF_GENERATION_TIMEOUT, ProofErrorSeverity.MEDIUM, ProofErrorRecoverability.RETRYABLE, context, originalError);
        }
        // Network errors
        if (errorMessage.includes("network") || errorMessage.includes("fetch")) {
            return new ProofGenerationError(errorMessage, ProofErrorType.NETWORK_ERROR, ProofErrorSeverity.LOW, ProofErrorRecoverability.RETRYABLE, context, originalError);
        }
        // Witness errors
        if (errorMessage.includes("witness")) {
            return new ProofGenerationError(errorMessage, ProofErrorType.WITNESS_PREPARATION_FAILED, ProofErrorSeverity.MEDIUM, ProofErrorRecoverability.RETRYABLE, context, originalError);
        }
        // Default to internal error
        return new ProofGenerationError(errorMessage, ProofErrorType.INTERNAL_ERROR, ProofErrorSeverity.HIGH, ProofErrorRecoverability.RETRYABLE, context, originalError);
    }
}
