 ...
 */
export async function encryptString(plaintext: string, signature: string): Promise<string> {
 ...
 */
export async function decryptString(ciphertext: string, signature: string): Promise<string> {
  const key = await deriveKeyFromSignature(signature);
