<script lang="ts">
  ...
  function calculatePayout(scoreValue: number, airdropConfig: typeof $airdrop): number {
    ...
    const normalizedScore = (clampedScore - airdropConfig.floor) / (airdropConfig.cap - airdropConfig.floor);
...
<div class="max-w-2xl mx-auto">
  ...
  {#if hasConnectionError}
    ...
    <div class="card bg-[var(--accent-error)]/10 border border-[var(--accent-error)]/30">
      <div class="text-center">
        <div class="text-[var(--accent-error)] font-medium mb-2">
          Connection Failed
        </div>
  ...
  {:else if !isConnected}
    ...
    <div class="card bg-[var(--accent-warn)]/10 border border-[var(--accent-warn)]/30 text-center">
      <div class="text-[var(--accent-warn)] font-medium mb-2">
    ...
    <div class="card">
      <div class="text-center mb-6">
        ...
        <p class="text-[var(--fg-secondary)] mb-4">
          ...
          but you need at least <strong>{(($airdrop.floor || 0) / 1e6).toFixed(3)}</strong> to be eligible.
        </p>
        
        {#if $walletMock.enabled}
          <div class="text-sm text-[var(--fg-muted)] bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg mb-6">
            <strong>Demo Mode:</strong> This user has a "{reputationTier}" reputation tier. 
            Use the mock controller (gear icon) to switch to a different user type.
          </div>
        {/if}
      </div>

      <div class="space-y-4">
        <h3 class="text-lg text-[var(--fg-primary)] font-semibold">How to increase your reputation:</h3>
        <div class="space-y-3">
          <ChecklistItem text="Verify your personhood with Worldcoin or similar providers" />
          <ChecklistItem text="Connect your developer accounts (GitHub, GitLab) to showcase contributions" />
  ...
  {:else}
    ...
    <div class="space-y-6">
      ...
      <div class="card bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700">
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1">
            <h3 class="text-lg font-semibold text-green-800 dark:text-green-200">You're Eligible!</h3>
            <p class="text-green-700 dark:text-green-300">
              Your reputation score of <strong>{(($score.value || 0) / 1e6).toFixed(3)}</strong> qualifies you for approximately <strong>{expectedPayout.toFixed(0)} tokens</strong>.
            </p>
            {#if $walletMock.enabled}
